...
export class PathSum {
    ...
    public static findPath(triangle: number[][]): number {
        let depth = triangle.length - 1;
        for (let row = depth; row >= 0; row--) {
            for (let j = 0; j < triangle[row].length; j++) {
                let isBottom = !triangle[row + 1];
                if (!isBottom) {
                    let cell = triangle[row][j];
                    let left = triangle[row + 1][j];
                    let right = triangle[row + 1][j + 1];
                    triangle[row][j] = cell + Math.max(left, right);
                }
            }
        }
        return triangle[0][0];
